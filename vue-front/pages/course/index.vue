      ...
      <section class="c-sort-box">
        <section class="c-s-dl">
          <dl>
            ...
            <dd class="c-s-dl-li">
              <ul class="clearfix">
                <li :class="{active:oneIndex==index}">
                  <a title="全部" href="#" @click="resetQuery()">全部</a>
                </li>
...
<script>
  ...
  export default {
    ...
    methods: {
      //查询第一页数据
      initCourseFirst() {
        courseApi.getCourseList(1, 8, this.searchObj).then(reponse => {
          this.data = reponse.data.data
        })
      },
      ...
      //分页
      gotoPage(page) {
        courseApi.getCourseList(page, 8, this.searchObj).then(reponse => {
          this.data = reponse.data.data
        })
      },

      //点击某个一级分类，查询出对应的一级分类的课程和二级分类类目
      searchOne(subjectParentId, index) {

        //把传递index赋值给oneIndex，为了active样式生效
        this.oneIndex = index
